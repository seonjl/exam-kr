"""g2 deep sample mismatch (19건) B1 재검수."""
import json
import re
import subprocess
...
PROMPT = """너는 한국 공인중개사 2차 시험 채점관이다. 아래 문항만 보고 정답을 신중하게 판단하라.
기존 해설/정답 키 참고 금지. 본문과 선택지만 근거.
법령 시점에 따라 답이 달라질 수 있으면 ambiguous=true.
...
def main():
    rows = json.loads((OUT/"no_conclusion_sample_g2_deep.json").read_text("utf-8"))
    cands = [r for r in rows if r.get("verdict")=="mismatch"]
    print(f"g2 deep mismatch 재검수: {len(cands)}건")
    results = []
    for i,c in enumerate(cands, 1):
        fl,num = c["qid"].split("#")
        q = find_q("g2", fl, int(num))
    ...
    (OUT/"g2_deep_verified.json").write_text(json.dumps(results, ensure_ascii=False, indent=2),"utf-8")
    confirmed = [r for r in results if r.get("verdict_final")=="confirmed"]
    print(f"\n양차 합의: {len(confirmed)}건")
    for r in confirmed:
