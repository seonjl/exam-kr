"""s2 deep sample mismatch B1 재검수."""
import json
import re
import subprocess
...
PROMPT = """너는 한국 사회조사분석사 2급 시험 채점관이다. 아래 문항만 보고 정답을 신중하게 판단하라.
...
def main():
    rows = json.loads((OUT/"no_conclusion_sample_s2_deep.json").read_text("utf-8"))
    ...
    (OUT/"s2_deep_verified.json").write_text(json.dumps(results, ensure_ascii=False, indent=2),"utf-8")
