"""g2 deep2 mismatch B1 재검수."""
import json, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent.parent
DATA = ROOT/"data"; OUT = DATA/"audit"
PROMPT = """너는 공인중개사 2차 시험 채점관이다. 본문과 선택지만 보고 신중히 판단.
법령 시점 의존 시 ambiguous=true.
[과목] {subject}
[문제] {question}
[선택지]
{choices}
JSON: {{"answer": 1-{n}, "confidence": 0.0-1.0, "ambiguous": true|false, "reason": "100자"}}"""
...
def find_q(fl, num):
    doc = json.loads((DATA/"g2"/f"{fl}.json").read_text("utf-8"))
    for q in doc["questions"]:
        if q["number"]==num: return q

def main():
    rows = json.loads((OUT/"no_conclusion_sample_g2_deep2.json").read_text("utf-8"))
    ...
    for i,c in enumerate(cands,1):
        ...
        if q is None or q.get("known_defect"):
            results.append({**c,"skip":"defect"}); print(f"  [{i}] {c['qid']} skip"); continue
        ...
        if first is None or sp is None: v = "incomplete"
        elif ambig: v = "ambiguous"
    ...
    (OUT/"g2_deep2_verified.json").write_text(json.dumps(results,ensure_ascii=False,indent=2),"utf-8")
