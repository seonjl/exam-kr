...
한국산업인력공단 시행 **사회조사분석사 2급** 필기 기출을 모바일에서 슬라이드로 풀이/해설할 수 있는 정적 웹앱.
...
webapp/
  ...
  icon.svg               앱 아이콘
data/
  sessions.json          회차 인덱스 (46회차, 2000 ~ 2021)
  s2_YYYYMMDD.json       회차별 문제/보기/정답/해설 JSON
scripts/
  fetch.py               기출 데이터 수집 스크립트
  scrub.py               텍스트 정리 유틸리티
  enrich.py              해설 AI 증강 스크립트
```
...
- **홈(기출)**: 연도별 46회차 리스트. 진도/완료 상태가 점으로 표시.
- **즐겨찾기**: 북마크한 문제 전체 회차에서 모아보기.
- **오답노트**: 풀이 중 틀린 문제 자동 수집.
- **설정**: 테마(자동·밝게·어둡게), 진도 내보내기(JSON), 초기화.

### 풀이 화면
- **풀이 모드**: 보기 탭 → 즉시 정답·오답 판정 + 해설 펼침.
- **해설 모드**: 정답·해설 바로 노출. 리뷰용.
- **가로 스와이프**: 이전/다음 문제 (스크롤-스냅, 키보드 ←/→ 지원).
- **문제 패드 시트**: 하단 중앙 버튼 탭 → 100개 번호 그리드에서 정답/오답/북마크 상태 한눈에.
...
- 본문 UI: Pretendard Variable + Noto Sans KR
- 숫자/레이블: Space Mono
...
- 계정·로그인 없음. 앱을 삭제하거나 설정 → 초기화하면 전부 사라짐.
- 진도 내보내기로 JSON 백업 가능.
